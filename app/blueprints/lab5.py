...
def db_connect():
    ...
    try:
        # Проверяем, на PythonAnywhere мы или локально
        if path.exists('/home/mynameisdmitry/web-back-labs/database.db'):
            # На PythonAnywhere
            db_path = '/home/mynameisdmitry/web-back-labs/database.db'
        else:
            # Локально
...
def get_login_id():
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ: у sqlite3.Row нет метода .get(), используем проверку ключа
        real_name = row['real_name'] if 'real_name' in row else ''
        return login_value, row['id'], real_name
    except Exception:
        if conn and cur:
            try:
                db_close(conn, cur)
    ...
    try:
        ...
        if conn and cur:
            try:
                ...
                pass
        return render_template('lab5/users.html', users=[], error=f"Ошибка базы данных: {e}")
